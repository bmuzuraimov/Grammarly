...
class Sign extends Model
{
    ...
    private function set_session($user_id, $session_key, $log_ip)
    {
        ...
        $result = $this->db->query('INSERT INTO `user_sessions` (`userid`, `authKey`, `log_ip`, `time`) VALUES (:user_id, :session_key, :log_ip, :time)', $params);
        return $result;
    }
    ...
    private function update_session($user_id, $session_key, $log_ip)
    {
        ...
        $result = $this->db->query('UPDATE `user_sessions` SET `authKey` = :session_key, `log_ip` = :log_ip, `time`=:time WHERE `userid` = :user_id', $params);
        return $result;
    }
}